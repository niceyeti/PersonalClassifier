...
void PersonalClassifier::matrixToFile(string& fname, const Matrix& matrix)
{
  ...
  string ostr;
  fstream matrixFile(fname.c_str(), ios::out);
  ...
  ostr.reserve(1 << 10); //reserve space in output string since we will only write to file periodically, when string gets sufficiently large
  ...
  for(outer = matrix.begin(); outer != matrix.end(); ++outer){
    for(inner = outer->second.begin(); inner != outer->second.end(); ++inner){
      U32 my_id = inner->first;
      ostr += std::to_string((unsigned long long)my_id);
      ostr += ":";
      ostr += m_wordIdManager->idToString(inner->first);
      ostr += "\t";
      ostr += std::to_string((long double)inner->second);
    }

    if(ostr.size() > 8192){
      matrixFile.write(ostr.c_str(),ostr.size());
      ostr.clear();
    }
  }
...
//Simply sums all of the components of a vector
double PersonalClassifier::doSum(DotProductVector& dotProduct)
{
  double ret = 0.0;

  for(int i = 0; i < dotProduct.size(); i++){
    ret += dotProduct[i].second;
  }
...
/*
  From ./aaa/bbb/file.txt returns ./aaa/bbb/ (with the terminal slash included)
*/
string PersonalClassifier::getDirName(const string& fullFilePath)
{
  return fullFilePath.substr(0, fullFilePath.find_last_of('/'));
}

//tiny utility to strip only the filename from some path: /dass/fsfs/fsdfds/fdsf/file.txt returns "file.txt"
string PersonalClassifier::getFileName(const string& fullFilePath)
{
  return fullFilePath.substr( fullFilePath.find_last_of('/') + 1);
}

//
void PersonalClassifier::testPerson(const string& personalTrainingFile, Matrix& testMatrix, DotProductVector& dotProduct)
{
  Matrix personalMatrix;

  //TODO: check if file already exists; if so, open and rebuild from it instead of rebulding the sim model

  //Build personal model from scratch
  //build the personally-trained matrix
  buildSimilarityModel(personalTrainingFile, personalMatrix);
  //write the model to file (far faster to read the processed files than to rebuild from scratch)
  string ofile = getDirName(personalTrainingFile) + "matrix.txt";
  matrixToFile(ofile,personalMatrix);

  //take the dot product of both to get the classifier
  matrixDotProduct(personalMatrix,testMatrix,dotProduct);
  printDotProductVector(dotProduct);
  cout << "testPerson() completed for " << personalTrainingFile << endl;
}

/*
  Main public method: given an input directory containing a bunch of personnel-text samples,
  build models for each one, and compare with the text in testSampleFile using the classifier
  similarity metric.

  TODO: Read Hadoop books.
  Lookup matrix similarity measures, dot product reduction. For the weighted method, I can only think
  of weighting each input by the term's relative frequency, and then normalizing. But this would require keeping
  the wordIndexTable around for longer.

*/
void PersonalClassifier::classify(const string& personnelDir, const string& testSampleFile)
{
  int i, maxIndex;
  double sum, maxSum, weightedSum;
  vector<string> absFilePaths;
  Matrix sampleMatrix;
  DotProductVector dotProduct;
  vector<pair<string,double> > results;

  //get list of personnel files
  //getFileList(absFilePaths,personnelDir);
  absFilePaths = {"dasovich/parsedSent.txt","kean/parsedSent.txt","germany/parsedSent.txt","shackleton/parsedSent.txt","jones/parsedSent.txt"}; 
  for(i = 0; i < absFilePaths.size(); i++){
    absFilePaths[i] = personnelDir + absFilePaths[i];
    cout << "file: " << absFilePaths[i] << endl;
  }

  //build the matrix for the test sample
  buildSimilarityModel(testSampleFile, sampleMatrix);
  
  maxSum = 0.0; maxIndex = 0;
  for(i = 0; i < absFilePaths.size(); i++){
    testPerson(absFilePaths[i],sampleMatrix,dotProduct);
    sum = doSum(dotProduct);
    if(sum > maxSum){
      maxIndex = i;
      maxSum = sum;
    }
    //weightedSum = doWeightedSum(dotProduct);
    results.push_back( pair<string,double>(absFilePaths[i],sum) );

    dotProduct.clear();
  }
  cout << "Given sample file " << testSampleFile << " max occurred for " << maxIndex << ":" << absFilePaths[maxIndex] << endl;
  
  for(i = 0; i < results.size(); i++){
    cout << results[i].first << "  dot product sum: " << results[i].second << endl;
  }
}
